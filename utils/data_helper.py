...
class Ambisonic(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scene_a = self.data[scene_name]['A']
        scene_b = self.data[scene_name]['B']
        a_ran = self.data[scene_name]['a_list'][sample_idx]
        b_ran = self.data[scene_name]['b_list'][sample_idx]

        label = scene_b[:self.args.num_out_chan, b_ran[0]:b_ran[1]]
        label = label / (np.max(np.abs(label)) + self.eps)

        image = scene_a[:, a_ran[0]:a_ran[1]]
        image = image / (np.max(np.abs(image)) + self.eps)
        return image, label
