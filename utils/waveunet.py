...
from utils.crop import centre_crop
from utils.resample import Resample1d
from utils.conv import ConvLayer
...
class Waveunet(nn.Module):
    def __init__(self, num_in_chan, num_channels, num_out_chan, kernel_size, target_output_size, conv_type, res, depth=1, strides=2):
        ...
        for instrument in model_list:
            ...
            for i in range(self.num_levels - 1):
                in_ch = self.num_in_chan if i == 0 else num_channels[i]
    ...
    def get_int_embd(self, x, module):
        '''
        A forward pass through a single Wave-U-Net (multiple Wave-U-Nets might be used, one for each source)
        :param x: Input mix
        :param module: Network module to be used for prediction
